...
func TestNewRetriever(t *testing.T) {
	...
	if retriever == nil {
		t.Fatal("Expected retriever to be created, got nil")
	}
	
	if retriever.client != mockESClient {
		t.Error("Expected ES client to be set correctly")
	}
	
	if retriever.embeddingClient != mockEmbeddingClient {
		t.Error("Expected embedding client to be set correctly")
	}
	
	// Test default configuration
	if retriever.bm25Weight != 0.3 {
		t.Errorf("Expected default BM25 weight 0.3, got %f", retriever.bm25Weight)
	}
	
	if retriever.vectorWeight != 0.7 {
		t.Errorf("Expected default vector weight 0.7, got %f", retriever.vectorWeight)
	}
	
	if retriever.mmrLambda != 0.7 {
		t.Errorf("Expected default MMR lambda 0.7, got %f", retriever.mmrLambda)
	}
}
